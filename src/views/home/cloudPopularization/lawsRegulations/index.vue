  ...
  <div class="page">
    <div>
      ...
      <div class="content">
        <div class="box">
          <!-- 横的导航栏 -->
          <Menu :menu-list="menuList" @choice="choice" />
          <div>
            <div v-for="(item,index) in stripList" :key="index + '1'" class="strip" v-bind="item" @click="handleDetails(item)">
              <Strip :type="3" :title="item.popularizationSendDept" :content="item.popularizationTitle" :date="item.popularizationSendTime" />
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    choice(e) {
      console.log('item', e)
      if (e.name === '联动单位') { this.$router.push('/disputeResolution') }
      if (e.name === '解纷组织') { this.$router.push('/disputeResolutionOrganization') }
      if (e.name === '解纷员') { this.$router.push('/disputeBreaker') }
    },
    // 加载数据
    async getList() {
      const params = {
    ...
    handleDetails(item) {
      ...
      this.$router.push(`/lawsRegulations/details?id=${item.popularizationId}`)
    },
    // 页码选择
    handlePageNum(e) {
      this.pageNumber = e
