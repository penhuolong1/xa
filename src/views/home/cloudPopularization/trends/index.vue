        ...
        <div class="box">
          <div>
            <div v-for="(item,index) in stripList" :key="index" class="strip" v-bind="item" @click="handleDetails(item)">
              <Strip :type="1" :title="item.popularizationSendDept" :content="item.popularizationTitle" :date="item.popularizationSendTime" />
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    handleDetails(item) {
      ...
      this.$router.push(`/trends/details?id=${item.popularizationId}`)
    },
    // 页码选择
    handlePageNum(e) {
      this.pageNumber = e
