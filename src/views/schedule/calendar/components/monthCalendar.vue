  ...
  <div class="flex">
    <div class="case-detail-left">
      <div v-for="item in detailList" :key="item.schedulingId" class="case-detail-item">
        ...
        <div class="other">
          <div class="time1">
            <Icon class="icon" type="clock-circle" />{{ item.startTime }}
          </div>
      ...
      <Calendar v-model="monthTime" @select="selectCalendar">
        ...
        <ul slot="dateCellRender" slot-scope="value" class="events">
          <li v-for="item in getListData(value)" :key="item.schedulingId">
            <div class="scheduling-item">{{ item.number || item.preNumber }}({{ item.names }})</div>
          </li>
        </ul>
      </Calendar>
    </div>
  ...
  methods: {
    ...
    selectCalendar(value) {
      ...
      this.detailList = list.map(item => {
        item.time = time
        item.startTime = item.schedulingStartime ? item.schedulingStartime.split(' ')[1] : ''
        return item
      })
