  ...
  >
    <div class="pop-wrapper">
      <Form :form="form" :label-col="{ span: 8 }" :wrapper-col="{ span: 12 }">
        <FormItem label="诉前调查时间">
          <DatePicker
            v-decorator="['schedulingTime', { rules: [{ required: true,message:'预约诉前调查时间不能为空'}] }]"
            type="date"
            :show-time="{
              'minute-step': 30,
              'second-step': 60,
...
export default {
  ...
  methods: {
    ...
    changeSmsNotice(e) {
      this.isSmsNotice = e.target.value
    },
    onConfirm() {
      this.form.validateFields(err => {
        ...
        params.lawCaseId = this.lawCase.lawCaseId
    ...
    getData() {
      ...
      // 获取当事人
      litigantNameByCaseId({ caseId: this.lawCase.lawCaseId }).then(({ code, data }) => {
        if (code === 200) {
