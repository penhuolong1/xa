...
import { quitRoom, quitSfRoom } from '@/api/rtc/index.js'
  ...
  methods: {
    back() {
      if (this.$route.query.type == 2 || this.$route.query.type == 4) {
        quitSfRoom({ sfLawCaseId: this.getSfLawCaseId }).then(({ code, msg }) => {
          if (code === 200) {
        ...
        quitRoom({ lawCaseId: this.getLawCaseId }).then(({ code, msg }) => {
          if (code === 200) {
            this.$message.success(msg)
            localStorage.removeItem('xaRtcToken')
            this.$router.go(-1)
