      ...
      <Form :form="form" :label-col="{ span: 6 }" :wrapper-col="{ span: 12 }">
        ...
        <div v-if="type=='1'">
          ...
          <Tribunal
            ...
            :form-option="{ rules: [{ required: true,message:'请选择调解地点'}],initialValue:selectedRows[0].casePreMediateDate }"
            @change="changePlace"
          />
        </div>
        <!-- 待正式立案 -->
        <div v-if="type=='2'">
          ...
          <FormItem label="诉前结案时间">
            <DatePicker v-decorator="['preFinishCaseTime',{ rules: [{ required: true , message: '请选择诉前结案时间'}] },] " />
          </FormItem>
        </div>
        <!-- 待庭室确认 -->
        <div v-if="type=='3'">
        ...
        <div v-if="!needConform&&(type==1||type==3)">
  ...
  methods: {
    ...
    nextConfirm() {
      ...
      this.form.validateFields(err => {
        ...
        if (this.isWhole) {
          params.process = params.nextProcess
