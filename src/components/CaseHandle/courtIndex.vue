<template>
  ...
  <div class="case-handle-wrapper">
    <div class="case-handle-wrapper-left">
      ...
      <MediationForm v-if="getSfCaseInfo.state==='调解中' && getUserInfo.loginRole.roleKey === roleTypeMap['调解员'] && getIsDeal === 0" :is-deal="isDeal" @done="getInfo" />  <!-- 待接收 -->
      <CourtMediationForm v-if="getSfCaseInfo.state==='调解中' && getUserInfo.loginRole.roleKey === roleTypeMap['调解员'] && (getIsDeal === 1 || getIsDeal === 2)" :is-deal="isDeal" @done="getInfo" />  <!-- 已接收 -->
      ...
      <Empty v-if="getSfCaseInfo.state==='待指派调解' && getIsDeal === 4" class="empty" description="已退回" />
      <Empty v-if="getSfCaseInfo.state==='待指派调解' && getIsDeal === 5" class="empty" description="已下发" />
...
import AssignContent from './courtAssignContent/index.vue'
import MediationForm from './mediationForm/index.vue'
import CourtMediationForm from './courtMediationForm/index.vue'
