    ...
    <div class="form-right">
      <div class="form">
        <span v-if="getIsDeal === 0 || menuIndex === 1">
          ...
          <CollaborateForm v-if="menuIndex === 1" />
          <Form1 v-if="menuIndex === 2" ref="form1" :label-col="4" :schema="schema1" @change="change" />
    ...
    return {
      ...
      schema1: [
        ...
        },
        {
          ...
          field: 'closeCaseDate',
          showTime: true,
          formOption: { rules: [{ required: true, message: '请输入理由' }], initialValue: this.format(new Date(), 'YYYY-MM-DD H:m:s') }
        },
        {
          label: '理由',
          type: 'textArea',
      ...
      return [
        {
          ...
          formOption: { },
          vIf: () => {
            return this.accept && this.getUserInfo.loginRole.roleKey !== roleTypeMap['调解组织']
          }
        },
        {
          label: '调解员',
        ...
        //   type: 'textArea',
        //   field: 'opinion',
        //   formOption: { rules: [{ required: true, message: '请输入退回理由' }] },
        //   vIf: () => {
        //     return !this.accept
        //   }
        // }
    ...
    submit() {
      ...
      if (this.menuIndex === 0) {
        this.$refs.form.getFieldsValue().then(res => {
          ...
          params.doType = 5
          params.goalDeptId = res.goalDeptId || this.getUserInfo.user.deptId
