    ...
    <h3>
      ...
      <div class="img-wrapper">
        ...
        <Tooltip placement="top">
          ...
          <!-- 根目录的parentid为0 -->
          <img src="../../../assets/icons/addFile.png" alt="" @click="add({id:null})">
        </Tooltip>
      </div>
    </h3>
    <div>
      <Tree :tree-data="treeData" :replace-fields="replaceFields" @select="selectTreeFun">
        <template slot="custom" slot-scope="item">
          <div class="tree-item">
            <div class="txt">
              <div v-show="!item.edit" @dblclick="edit(item)">{{ item.name }}</div>
...
<script>
import { listOfCase, editCatalog, delCatalog, downArchiveFile } from '@/api/sfBook/index.js'
...
export default {
  ...
  methods: {
    ...
    getTreeDate() {
      ...
      listOfCase(params).then(({ code, data }) => {
        if (code === 200) {
    ...
    handleOk() {
      ...
      editCatalog(params).then(({ code, msg }) => {
        if (code === 200) {
    ...
    dealTree(list, id, name) {
      ...
      newList = list.map(item => {
        if (item.childrenList && item.childrenList.length !== 0) {
          item.childrenList = this.dealTree(item.childrenList, id, name)
        }
        if (item.id === id) {
          item.edit = !item.edit
          if (name) {
    ...
    del(item) {
      this.$confirm({
        ...
        onOk: () => {
          ...
          delCatalog(params).then(({ code, msg }) => {
            if (code === 200) {
              this.$message.success(msg)
              this.getTreeDate()
            }
