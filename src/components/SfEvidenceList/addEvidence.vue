  ...
  >
    <Form :form="form" :label-col="{ span: 8 }" :wrapper-col="{ span: 12 }">
      <SfLitigant
        label="举证人"
        field="sfLitigantId"
        :form-option="{rules: [{ required: true,message:'请选择举证人' }],initialValue: detailObj ? detailObj.sfLitigantId:undefined}"
        placeholder="请选择举证人"
      />
      <FormItem label="证据名称">
...
import SfLitigant from '@/components/Common/SfLitigant.vue'
...
export default {
  ...
  methods: {
    show(e) {
      this.$nextTick(() => {
        this.detailObj = e
        if (e.materialFiles && e.materialFiles.length > 0) {
          this.defaultFileList = e.materialFiles.map(item => {
            return {
              uid: item.id,
              id: item.id,
              name: item.fileName,
