  ...
  <div>
    ...
    >
      <div slot="files" slot-scope="record">
        <a v-for="item in record.materialFiles" :key="item.id" style="display:block;" @click="preview(item, record.materialFiles)">{{ item.fileName }}</a>
      </div>
...
import { listSfMaterialByFileType, delEvidence } from '@/api/sfAddcase/index'
...
const columns = [
  ...
  { title: '提供者', dataIndex: 'sfLitigantName', key: 'sfLitigantName', align: 'center' },
  { title: '证据名称', dataIndex: 'name', key: 'name', align: 'center', width: 300 },
  { title: '证据材料', align: 'center', scopedSlots: { customRender: 'files' }, width: 250 },
  { title: '证明对象', dataIndex: 'eviProve', key: 'eviProve', align: 'center', width: 300 },
...
export default {
  ...
  methods: {
    ...
    getLitigant() {
      ...
      this.list = []
      listSfMaterialByFileType({ sfLawCaseId: this.getSfLawCaseId, fileCode: 1 }).then(({ code, data }) => {
        if (code === 200) {
