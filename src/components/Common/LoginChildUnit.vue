<template>
  <!-- 诉非登录角色下级单位 -->
  <span class="w-full">
    <FormItem v-if="isForm" ref="formItem" :label="label">
      <Select v-decorator="[field, formOption]" :placeholder="placeholder" :disabled="disabled" allow-clear show-search @change="change">
        <SelectOption v-for="item in list" :key="item.deptId" :value="item.deptId">{{ item.deptName }}</SelectOption>
      </Select>
    </FormItem>
    <Select v-else :default-value="defaultValue" :placeholder="placeholder" :disabled="disabled" allow-clear show-search @change="change">
      <SelectOption v-for="item in list" :key="item.deptId" :value="item.deptId">{{ item.deptName }}</SelectOption>
    </Select>
  </span>
</template>

<script>
import { Select, Form } from 'ant-design-vue'
import { listChildOfUnit } from '@/api/sfAddcase/index'

export default {
  components: {
    FormItem: Form.Item,
    Select,
    SelectOption: Select.Option
  },
  props: {
    isForm: {
      type: Boolean,
      default: true
    },
    form: {
      type: Object,
      default: () => {}
    },
    label: {
      type: String,
      default: ''
    },
    disabled: {
      type: Boolean,
      default: false
    },
    field: {
      type: String,
      default: ''
    },
    formOption: {
      type: Object,
      default: () => {}
    },
    placeholder: {
      type: String,
      default: '请选择'
    },
    isNoReset: { // 切换当事人的时候不需要情况以有的选择
      type: Boolean,
      default: false
    },
    defaultValue: {
      type: String,
      default: undefined
    },
    value: {
      type: String,
      default: undefined
    },
    codes: {
      type: String,
      default: '11,12' // 10中心11单位12街道13组织 多个逗号隔开
    },
    ancestorId: { // 上级部门id
      type: String,
      default: ''
    },
    place: { // 1法院得机构组织 2 诉非的单位组织
      type: Number,
      default: 2
    }
  },
  data() {
    return {
      list: []
    }
  },
  computed: {
  },
  watch: {
    getLitigantId() {
      this.getData()
    }
  },
  created() {
    this.getData()
  },
  mounted() {

  },
  methods: {
    getData() {
      // 获取下级部门
      listChildOfUnit({ ancestorId: this.deptId, codes: this.codes, place: this.place }).then(({ code, data }) => {
        if (code === 200) {
          if (!data || data.length === 0) {
            return
          }
          this.list = data
        }
      })
    },
    change(e) {
      this.$emit('change', e)
      this.$emit('input', e)
    }
  }
}
</script>

<style scoped lang="scss">

</style>
