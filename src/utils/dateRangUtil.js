...
var dateRangeUtil = (function() {
  ...
  * 获得本周的所有日期
  */
  this.getCurrentWeeks = function(date) {
    // 起止日期数组
    var startStop = []
    // 获取当前时间
    var currentDate = date || this.getCurrentDate()
    // 返回date是一周中的某一天
    var week = currentDate.getDay()
    ...
    for (let i = 0; i < 7; i++) {
      const day = new Date(monday.getTime() + (i * millisecond))
      const obj = {
        time: day.format('yyyy-MM-dd'),
        weekName: '星期' + weekMap[i]
      }
      startStop.push(obj)
    }
    return startStop
  ...
  */
  this.getCurrentYear = function() {
    ...
    // 本年最后一天
    var currentYearLastDate = new Date(currentYear, 12, 31)
    // 添加至数组
