...
function sendCmd (cmd_name, paraObj, resultCallback) {
  ...
  $.ajax({
    ...
    success: function (data, textStatus) {
      ...
      bSync = true
      nResult = data.result
      if (resultCallback != '' && resultCallback != null) { resultCallback(nResult) }
      return nResult
    },
		 complete: function (XMLHttpRequest, textStatus) {
      if (XMLHttpRequest.readyState == '4') {
...
function check_ver (iVer, nextFunc) {
  ...
  $.ajax({
    ...
    success: function (data, textStatus) {
      // $("#StatusArea").append("--- " + data );
      nResult = data.result
      if (nextFunc != '' && nextFunc != null) { nextFunc(nResult) }
      return nResult
    },
		 complete: function (XMLHttpRequest, textStatus) {
      if (XMLHttpRequest.readyState == '4') {
...
function Storage (nDevID, uOPType, uSeek, pPwd, pValue, nextFunc) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (data.outbuf != null) { objResult.outbuf = decodeURIComponent(data.outbuf) }
      if (nextFunc != null) { nextFunc(objResult) }
    },
		 complete: function (XMLHttpRequest, textStatus) {
      if (XMLHttpRequest.readyState == '4') {
        // alert(XMLHttpRequest.responseText);
      ...
      if (data.outbuf != null) {
        objResult.outbuf = decodeURIComponent(data.outbuf)
			 ...
			 case	QNV_TOOL_CODETYPE:
				 if (nextFunc != null) { nextFunc(objResult.result) }
          return objResult.result
			 case QNV_TOOL_LOCATION:
				 {
					 if (nextFunc != null) { nextFunc(objResult.outbuf) }
